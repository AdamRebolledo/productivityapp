...
const NavShoppingCart = () => {
  ...
  const total =
    ctx?.products?.reduce((a: number, b: TProduct) => a + b.count, 0) > 9
      ? '9+'
      : ctx?.products?.reduce((a: number, b: TProduct) => a + b.count, 0)
  ...
  return (
    <div>
      <div className='w-full text-right p-4'>
        ...
        <div className='bg-yellow py-1 px-2 rounded-full text-white inline relative top-6 -left-16'>{total}</div>
      </div>
      {show && (
